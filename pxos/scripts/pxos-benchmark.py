...
def sanitize_payload(raw: Dict[str, Any]) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
    ...
    clean = {
        ...
        "critical_assessment": {
            ...
            "identified_frictions": [str(f)[:256] for f in (crit.get("identified_frictions") or []) if f][:5] if isinstance(crit.get("identified_frictions") or [], (list, tuple)) else [],
            "concrete_benefits": [str(b)[:256] for b in (crit.get("concrete_benefits") or []) if b][:5] if isinstance(crit.get("concrete_benefits") or [], (list, tuple)) else []
        }
    }
